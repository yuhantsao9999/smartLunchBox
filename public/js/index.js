window.addEventListener('load', () => {
    ...
    const data = {
        id: [1, 2, 3, 4],
        product_id: [1, 2, 3, 4],
        name: ['雞腿餐盒', '清雞腿餐盒', '打拋豬餐盒', '牛肉餐盒'],
        price: [100, 200, 150, 250],
    };
    if (data) {
        for (let i = 0; i < 2; i++) {
            fillInProduct('new' + i, data.product_id[i], i, data.name[i], data.price[i]);
        }
        for (let i = 0; i < 2; i++) {
            fillInProduct('rank' + i, data.product_id[i], i + 2, data.name[i + 2], data.price[i + 2]);
        }
    }
});
